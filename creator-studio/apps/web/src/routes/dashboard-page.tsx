import { ArrowRight, CircleCheck, FolderKanban, ListTodo, Plus, RefreshCw, Sparkles } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl">
      ...
      <div className="mt-7 grid gap-px overflow-hidden rounded-lg border border-border bg-border sm:grid-cols-3">
        <div className="bg-surface p-5"><p className="text-xs uppercase tracking-[0.16em] text-muted">{t('dashboard.currentProjects')}</p><p className="mt-3 font-display text-3xl font-semibold">{projects.length}</p></div>
        <div className="bg-surface p-5"><p className="text-xs uppercase tracking-[0.16em] text-muted">{t('dashboard.activeTasks')}</p><p className="mt-3 font-display text-3xl font-semibold">{tasks.filter((task) => ['queued', 'running', 'waiting_review'].includes(task.status)).length}</p></div>
        <div className="bg-surface p-5"><p className="text-xs uppercase tracking-[0.16em] text-muted">{t('dashboard.workspaceStatus')}</p><p className="mt-3 flex items-center gap-2 text-sm font-semibold text-primary"><CircleCheck className="h-4 w-4" />{t('dashboard.localReady')}</p></div>
      </div>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1.35fr)_minmax(18rem,.65fr)]">
        <section aria-labelledby="recent-projects-heading">
          ...
          {recentProjects.length > 0 ? <div className="mt-4 grid gap-px overflow-hidden rounded-lg border border-border bg-border sm:grid-cols-2">{recentProjects.map((project) => <Link className="group bg-surface p-5 hover:bg-elevated" key={project.id} to={`/projects/${project.id}/overview`}><span className="text-xs uppercase tracking-[0.16em] text-muted">{formatContentType(project.contentType, t)}</span><h3 className="mt-6 font-display text-xl font-semibold group-hover:text-primary">{project.title}</h3><p className="mt-2 line-clamp-2 text-sm text-muted">{project.brief || t('dashboard.missingBrief')}</p><span className="mt-5 inline-flex items-center gap-1 text-xs font-semibold text-primary">{t('dashboard.openOverview')} <ArrowRight className="h-3.5 w-3.5" /></span></Link>)}</div> : null}
        </section>
        ...
        <section aria-labelledby="recent-tasks-heading">
          ...
          <Button className="mt-4 w-full" onClick={() => notify({ title: t('dashboard.workspaceReady'), description: t('dashboard.workspaceReadyDescription') })} variant="secondary"><Sparkles className="h-4 w-4" />{t('dashboard.previewNotification')}</Button>
        </section>
      </div>
    </div>
  )
