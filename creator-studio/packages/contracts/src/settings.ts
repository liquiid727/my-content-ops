...
export const saveProviderSettingSchema = z.object({ displayName: z.string().trim().min(1).max(100), enabled: z.boolean(), model: z.string().trim().max(100).optional(), credential: z.string().min(1).max(10_000).optional() }).strict()
