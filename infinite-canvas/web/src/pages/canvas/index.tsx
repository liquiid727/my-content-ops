...
export default function CanvasPage() {
    ...
    const enterProject = (id: string) => {
        ...
        navigate(`/canvas/${id}${query ? `?${query}` : ""}`);
    };
    ...
    useEffect(() => {
        if (!hydrated || !externalId || autoOpenRef.current) return;
        autoOpenRef.current = true;
        const storageKey = `studio-project:${externalId}`;
        const stored = window.localStorage.getItem(storageKey);
        const existing = stored && projects.some((project) => project.id === stored) ? stored : null;
        if (existing) {
            enterProject(existing);
            return;
        }
        const id = createProject(externalTitle || t("canvas.defaultTitle", { count: projects.length + 1 }));
        window.localStorage.setItem(storageKey, id);
        enterProject(id);
    }, [createProject, externalId, externalTitle, hydrated, projects, t]);

    if (hydrated && (externalId || mode === "new" || mode === "recent")) return <main className="flex h-full items-center justify-center bg-background text-sm text-stone-500">{t("canvas.opening")}</main>;
