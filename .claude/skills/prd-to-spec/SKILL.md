---
name: prd-to-spec
description: "Transform a PRD into a technical SPEC document — architecture, API design, data model, error handling, and implementation contracts. Reads from the .feature workspace (.feature/.feature-<NNN>-<slug>/.prd) and writes back to .feature/.feature-<NNN>-<slug>/.spec. Triggers on: prd-to-spec, prd to spec, prd转spec, 需求转设计, 需求转规格, generate spec from prd, design from prd, 技术方案, 设计方案."
user-invocable: true
---
...
与 `/prd`、`/to-issues` 共用 `.feature/` 工作区。每个 feature 一个目录，SPEC 写到与输入 `.prd` 相同的 feature 目录下：

```
.feature/
└── .feature-<NNN>-<slug>/
    ├── .prd        # 输入：需求文档 PRD
    ├── .spec       # 输出：技术规格 SPEC —— 本 skill 产出
    ├── .issues     # 本地 issue 列表
    └── .test       # 测试计划 / 测试文档
```

- 定位输入 `.prd` 后，输出固定写到同目录的 `.spec`（不新建目录）
...
1. **Locate PRD** — find the `.prd` in `.feature/` (or receive it directly)
...
6. **Save** — write final SPEC to `.feature/.feature-<NNN>-<slug>/.spec`
...
A. Feature dir path (e.g., .feature/.feature-001-priority-system)
B. Direct .prd path (e.g., .feature/.feature-001-priority-system/.prd)
C. Paste PRD content directly
D. Auto-detect: scan .feature/ for recent .prd files
```

If auto-detecting, list available PRDs and let the user choose:

```
Found PRDs in .feature/:
  1. .feature-001-priority-system/.prd (2024-03-15)
  2. .feature-002-user-auth/.prd (2024-03-10)
...
SPEC 默认保存到与输入 `.prd` 相同的 feature 目录，文件名为 `.spec`。不要新建目录。

```
SPEC 保存位置：.feature/.feature-<NNN>-<slug>/.spec（与 .prd 同目录）
```
...
| `.feature/` has no `.prd` | Ask user to provide a feature dir or `.prd` path, or paste PRD content |
...
```
/prd  →  /prd-to-spec  →  /to-issues  →  /loop-it-local
 │              │              │
...
- **/to-issues** decomposes PRD+SPEC into local Issues
- **/loop-it-local** implements local Issues one by one (review → note → local ship)
